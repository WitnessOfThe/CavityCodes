    ...
    Delta(1).Loops         = 400;                        %   Number of Looops that you disire
    Delta(1).dt            = 1E-4/2;                         %   Time ste[ in loops
    Delta(1).per_loop      = 10;                           %   How ma1000ny times per loop we will save the data
...
NN = 8;
Power(1:8)      = 0.05;
detunning(1:8)  = linspace(-0.6106E6,-0.608E6,8)*2*pi;
%detunning   = meshgrid(linspace(-0.595E6,-0.585E6,8)*2*pi,linspace(0.45,0.55,N);
...
%    plot(tong(3).t_max(ii,:),tong(3).U_max(ii,:))
    plot(2*real(L_L_Split(ii).Solution.Temporal.E_s(:,36)))
end
%%
ii = 32;
figure;
...
xlim([-pi,pi]);
subplot(2,2,4);plot(fftshift(L_L_Split(ii).Space.k),fftshift(10*log10(abs(L_L_Split(ii).Solution.Temporal.E_s(end,:)/L_L_Split(ii).Space.N).^2*2*pi),2 ));
xlim([-250,250]*3);
%%
%i = 36;33
    ii =3;
...
for i = 1:96
    L_L_save(i).Solution.Temporal.E_s = L_L_Split(i).Solution.Temporal.E_s(end,:);
end