...
%%
    ...
    Res.Stat(1).Par.Newton_tol       = 1E-10;  
    Res.Stat(1).Par.Turning          =    0;
    Res.Stat(1).Par.variable         = 'delta_e';  %%'Pump Power';
    ...
    Res.Stat(1).Par.Newton_iter      = 50;
    Res.Stat(1).Par.first_step       = 6E-2;
    Res.Stat(1).Par.max_step         = 6E-2;
    Res.Stat(1).Par.step_tol         = 1E-16;
    Res.Stat(1).Par.step_inc         = 1.1;  
    Res.Stat.Par.i_max               = 200;
     

 %%
 eps_vector =  -Res.CW.In.Do(2)*Res.CW.In.mu_bl ^2;%mu*(Res.CW.In.De(1) -Res.CW.In.Do(1)) +1/2*(mu^2*Res.CW.In.De(2)- Res.CW.In.Do(2)*(mu1^2+mu2^2));%;[0]*Res.CW.In.ko;
 dd         =  linspace(-0.5,1,12);% - 40      - eps_vector/Res.CW.In.ko+%(-1/2*(mu^2*Res.CW.In.De(2)- Res.CW.In.Do(2)*(mu1^2+mu2^2)))/Res.CW.In.ko(1); 
 W          = 1e-1;
            ...
            for ip =1:size(W,2)            
                tic
                 parfor id = 1:size(dd,2)
                     ...
                     R.CW.In.delta_o = (dd(id)*R.CW.In.ke+R.CW.In.eps)/2;%(dd(id)*R.CW.In.ko)/2;
                     %R.CW.In.delta_e2 = (dd(id)*R.CW.In.ko);
                     R.CW.In.W       = W(ip);
                     toc
%                     ResSave(id) = RunStartingSimple(R);
                      ResSave(id)= StartOPO(R);                   
                 end    
                toc
            end
...
function [R] =  StartOPO(R)

 %       R.CW                 = Chi23_Stat_OPO_AnalyticsTurings(R.CW);
        R.Stat.In            = R.CW.In;
        [R.Stat,CW] = Chi23_OPO_StartTuringFromAnal(R.CW,R.Stat);
%        [R.Stat]   = Chi23_OPO_StartTuringFromAnal(R.CW,R.Stat);
        R.Temp.In = R.Stat.In;
        R.Temp          = R.Temp.Met.Norm(R.Temp);   
