...
%%
 ...
 mu1 =  125;
 mu2 = -125;
 eps_vector =  0;%mu*(Res.CW.In.De(1) -Res.CW.In.Do(1)) +1/2*(mu^2*Res.CW.In.De(2)- Res.CW.In.Do(2)*(mu1^2+mu2^2));%;[0]*Res.CW.In.ko;
 dd         =  [-10:0.3:10];%(-1/2*(mu^2*Res.CW.In.De(2)- Res.CW.In.Do(2)*(mu1^2+mu2^2)))/Res.CW.In.ko(1); 
 W          =  40E-6;
        ...
        SPEPS = strcat('/home/dp710/DataGit/Chi23/Dynamics/CWScans/NoOPOStartEps',num2str(eps_vector(ieps)/Res.CW.In.ko),'/');
            ...
            for ip =1:size(W,2)            
                 ...
                 parfor id = 1:size(dd,2)
                     ...
                     R.CW.In.delta_o = 0;%(dd(id)*R.CW.In.ko)/2;
