...
%%
    ii = 997;
   Stat_2  =   BranchTurning([real(ifft(Res.Stat(ii).Sol.Psi_o)),imag(ifft(Res.Stat(ii).Sol.Psi_o)),real(ifft(Res.Stat(ii).Sol.Psi_e)),imag(ifft(Res.Stat(ii).Sol.Psi_e)),Res.Stat(ii).Sol.V/Res.Stat(ii).Space.N]*Res.Stat(ii).Space.N,x_0,Res.Stat(ii), 1);
...
%        V_zero_value(i)         = 
    ...
    plot(delta_vector,V_vector,'Color',[1,0,0],'Parent',ax(1));
...
%    plot(delta_vector,abs(Psi_hat_e(:,2)).^2,'Color',[0,0,1],'Parent',ax(1));
 %   plot(delta_vector(Stable),abs(Psi_hat_e(Stable,2)).^2,'Color',[0,0,1],'Parent',ax(1),'LineWidth',2);
  ...
  %  plot(delta_vector(Stable),V_vector(Stable),'Color',[0,0,0],'Parent',ax(2),'LineWidth',2);
    plot(delta_vector,max_psi.','Color',[1,0,0],'Parent',ax(2),'LineWidth',1,'MarkerSize',20);
    dd = delta_vector;
    dd(Stable==0) =NaN;
    mp =max_psi;
    mp(Stable==0) =NaN;
    plot(dd,mp.','Color',[1,0,0],'Parent',ax(2),'LineWidth',2,'MarkerSize',20);

    plot(delta_vector.','Color',[0,0,0],'Parent',ax(3),'LineStyle','-');
    plot(max_psi.','Color',[0,0,0],'Parent',ax(4),'LineStyle','-');
