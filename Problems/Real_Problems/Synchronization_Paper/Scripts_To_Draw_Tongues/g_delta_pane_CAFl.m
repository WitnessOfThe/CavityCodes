...
%%
  ...
  tic
  delta_vector  = linspace(-0.5,0.15,N);
  g_vector      = linspace(0,0.03,N);
  ...
  parfor i = 1:N
      ...
      for ii = 1:N
          ...
          SiN.CW.In.g = g_vector(ii)*2*pi*1E6;
...
%%
  L_L.CW.In.kappa   =  0.5E5*2*pi;%2*1E3*2*pi;%
  ...
  W_MI_vector_2_1 = zeros(NN,N_Mode/2);
     ...
     parfor i = 1:NN
          ...
          SiN.CW          = SiN.Met.T_MI (SiN.CW,N_Mode);
          SiN.CW          = SiN.Met.T_Syn(SiN.CW,N_Mode);      
          W_MI_vector_1_1(i,:)        = SiN.CW.In.g_MI(1,1:N_Mode/2)/(2*pi*1E6);
          W_MI_vector_2_1(i,:)        = SiN.CW.In.g_MI(2,1:N_Mode/2)/(2*pi*1E6);

          i
      end
%%
  L_L.CW.In.kappa   =  1E4*2*pi;%2*1E3*2*pi;%
  delta_vector_2 = linspace(min(delta_vector),max(delta_vector),NN);
  ...
  parfor i = 1:NN      
      ...
      SiN.CW          = SiN.Met.T_MI (SiN.CW,N_Mode);
      SiN.CW          = SiN.Met.T_Syn(SiN.CW,N_Mode);      
      W_MI_vector_1_2(i,:)        = SiN.CW.In.g_MI(1,1:N_Mode/2)/(2*pi*1E6);
      W_MI_vector_2_2(i,:)        = SiN.CW.In.g_MI(2,1:N_Mode/2)/(2*pi*1E6);
...
%%
  ...
  parfor i = 1:NN      
      ...
      W_MI_vector_1_3(i,:)     = SiN.CW.In.g_MI(1,1:N_Mode/2 )/(2*pi*1E6);
      W_MI_vector_2_3(i,:)     = SiN.CW.In.g_MI(2,1:N_Mode/2 )/(2*pi*1E6);
      W_Synch_vector_1_3(i,:)  = SiN.CW.In.g_Synch(1,1)/(2*pi*1E6);
      W_Synch_vector_2_3(i,:)  = SiN.CW.In.g_Synch(2,1)/(2*pi*1E6);
...
%%
    ...
    pp4 = pp4.changeAxisOptions('ColorMap', 'parula',...
        ...
        'g (MHz)','FontSize',15,'XLabelText','$\delta_0$ (GHz)','XLim',[min(delta_vector),max(delta_vector)]...
