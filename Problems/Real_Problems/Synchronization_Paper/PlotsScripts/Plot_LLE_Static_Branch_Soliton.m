function [tt_1,tt_2,tt_3,tt_4] = Plot_LLE_Static_Branch_Soliton(CW,Stat_Up,Stat_Dw,Flag,mu,Stat)
...
%%  
    ...
    lambda_v_U  = zeros(20,size(Stat_Up,2));
    ...
    lambda_v_D  = zeros(20,size(Stat_Dw,2));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %    Mode_P_U_z(i_d)   = abs(Stat_Up(i_d).Sol.Psi_k( Stat_Up(i_d).Space.k == 0 )).^2*Stat_Up(1).In.gamma;    
        ...
        lambda_v_U(:,i_d) = Stat_Up(i_d).Stab.E_values(1:20);
...
%         lambda_v_D(:,i_d) = Stat_Dw(i_d).Stab.E_values(1:20);        
...
%    tt_1         = tt_1.addData(X_D,Mode_P_D/Stat.In.kappa,'Color',[0,0,1]);
  ...
  %  tt_2         = proPlot(     X_D,real(lambda_v_D)/Stat.In.kappa,'Color',[0,0,1],'Marker','.','LineStyle','none');
    tt_2         =proPlot(X_U,real(lambda_v_U)/Stat.In.kappa,'Color',[1,0,0],'Marker','.','LineStyle','none');
  ...
  %  tt_3         = proPlot(     X_D,imag(lambda_v_D)/Stat.In.kappa,'Color',[0,0,1],'Marker','.','LineStyle','none');
    tt_3         =  proPlot(X_U,imag(lambda_v_U)/Stat.In.kappa,'Color',[1,0,0],'Marker','.','LineStyle','none');
  ...
  %  tt_4         = proPlot(     X_D,Veloc_v_D/Stat.In.kappa,'Color',[0,0,1]);
    tt_4         =proPlot(X_U,Veloc_v_U/Stat.In.kappa,'Color',[1,0,0]);
