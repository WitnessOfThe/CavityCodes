...
%%  
    ...
    ind_Min_y = find(10*log10(abs(fftshift(Psi_k(end,:),2)).^2*2*pi)> -50,1,'first');
    ind_Max_y = find(10*log10(abs(fftshift(Psi_k(end,:),2)).^2*2*pi)> -50,1,'last');
 ...
 %   tt_1 = tt_1.addData(Temp.Space.phi,abs( ifft(Temp.Space.N*Psi_k(floor(end/2),:))).^2 );
    tt_1 = tt_1.addData(Temp.Space.phi,abs( ifft(Temp.Space.N*Psi_k(end,:))).^2 );
 ...
 %   tt_2 = tt_2.addData(fftshift(Temp.Space.k),10*log10(fftshift( abs(Psi_k(floor(end/2),:)).^2*2*pi)),'PlotType','stem','BaseValue',-80,'Marker','none');
    tt_2 = tt_2.addData(fftshift(Temp.Space.k),10*log10(fftshift( abs(Psi_k(end,:)).^2         *2*pi)),'PlotType','stem','BaseValue',-80,'Marker','none');
