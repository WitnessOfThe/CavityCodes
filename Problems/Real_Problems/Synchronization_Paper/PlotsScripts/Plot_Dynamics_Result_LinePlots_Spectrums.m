...
%%  
    ...
    tt_1 = 0;
    ind_Min_y = find(10*log10(abs(fftshift(Psi_k(end-1,:),2)).^2*2*pi)> -70,1,'first');
    ind_Max_y = find(10*log10(abs(fftshift(Psi_k(end-1,:),2)).^2*2*pi)> -70,1,'last');
   ...
   tt_1 = tt_1.addData(Temp.Space.phi,abs( ifft(Temp.Space.N*Psi_k(end-1,:))).^2 );
    ...
    tt_2 = proPlot(     fftshift(Temp.Space.k),10*log10(fftshift( abs(Psi_k(1,:)).^2           *2*pi)),'PlotType','stem','BaseValue',-70,'Marker','none');
    tt_2 = tt_2.addData(fftshift(Temp.Space.k),10*log10(fftshift( abs(Psi_k(floor(end/2),:)).^2*2*pi)),'PlotType','stem','BaseValue',-70,'Marker','none');
    tt_2 = tt_2.addData(fftshift(Temp.Space.k),10*log10(fftshift( abs(Psi_k(end-1,:)).^2         *2*pi)),'PlotType','stem','BaseValue',-70,'Marker','none');
    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

   tt_1 = tt_1.changeAxisOptions('XLabelText','$\theta$',...
                        'YLabelText','Power (W)',...  
                        'FontSize',13,'XLim',[-pi,pi],'YTickLabelRotation',90 );
