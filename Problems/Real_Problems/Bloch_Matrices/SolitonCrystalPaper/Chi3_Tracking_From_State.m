...
%%
    ...
    P1      = tiledlayout(2,4,'TileSpacing','none','Padding','none','Parent',fstable);
    axb(1)  =  nexttile(P1,1,[1,1]);  
    axb(2)  =  nexttile(P1,2,[1,1]);  
    axb(3)  =  nexttile(P1,3,[1,1]);  
    ...
    Plot_Vector(R.Stat, 7,axb(1:4))
    Plot_Vector(R.Stat,-7,axb(5:8))
   clear fstable 
   clear P1
   clear axb 
...
%%
    ...
    Plot_Static_StabilityK(Stat_1,6,ax(1:2))
    Plot_Static_StabilityK(Stat_2,6,ax(1:2))
    Plot_Static_StabilityK(Stat_1,-6,ax(3:4))
    Plot_Static_StabilityK(Stat_2,-6,ax(3:4))
    ax(2).YLim = [-3.5,3.5];
    ax(4).YLim = [-3.5,3.5];
    ax(1).YLim = [0,0.01];
    ax(3).YLim = [0,0.01];
...
function  Plot_Static_Branch(Stat_1,ax)
    ...
    plot(delta,SmParam,'Color',[0,0,0],'LineWidth',1,'Parent',ax(2));
...
%    stem(fftshift(S.Space.k)/S.In.mu_bl,fftshift( 10*log10(abs(S.Sol.Psi_k).^2/max(abs(S.Sol.Psi_k).^2)) ),'Parent',ax(3),'Color',[0,0,0],'BaseValue',-100,'Marker','none','LineWidth',1);
    ...
    Vect = ifft(S.Stab.Vk(Ish).Vectors(1:S.Space.N,1));
    ...
    Vect = TransferToResontor(S.Stab.Vk(Ish).Vectors(1:S.Space.N,1),S.In.mu_bl);
    ...
    Vect = ifft(S.Stab.Vk(Ish).Vectors(S.Space.N+1:2*S.Space.N,1));    
    ...
    Vect = TransferToResontor(S.Stab.Vk(Ish).Vectors(S.Space.N+1:2*S.Space.N,1),S.In.mu_bl);
...
function  Plot_Static_StabilityK(Stat_1,k,ax)
    ...
    for i = 1:size(Stat_1,2)
        ...
        plot(Stat_1(i).Eq.delta*ones(size(ReLambda)),ReLambda...
                       ,'Color',[0,0,0],'LineStyle','none',...
                       'Marker','.','Parent',ax(1),'MarkerSize',15);
                   
        plot(Stat_1(i).Eq.delta*ones(size(ImLambda)),ImLambda...
                       ,'Color',[0,0,0],'LineStyle','none',...
                       'Marker','.','Parent',ax(2),'MarkerSize',15);
                   
        plot(Stat_1(i).Eq.delta*ones(size(UnstLambda)),real(UnstLambda)...
                       ,'Color',[1,0,0],'LineStyle','none',...
                       'Marker','.','Parent',ax(1),'MarkerSize',15);
                   
        plot(Stat_1(i).Eq.delta*ones(size(UnstLambda)),imag(UnstLambda)...
                       ,'Color',[1,0,0],'LineStyle','none',...
                       'Marker','.','Parent',ax(2),'MarkerSize',15);
