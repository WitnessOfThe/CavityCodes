...
%%
  Path = strcat('/home/dp710/Data_Storage/From_Bloch_Stationary/CaF_mu=',num2str(CaF.Stat(1).In.mu_bl),'_',num2str(CaF.Stat(1).In.D(2)/CaF.Stat(1).In.kappa),'_Upper_Branch_Scan_',CaF.Stat.Par.variable ,'_Start_delta=',num2str(CaF.Stat(1).In.delta/CaF.Stat(1).In.kappa),'_Power=',num2str(W_WStar));
  delete(gcp('nocreate'));
 parpool(36);
  parfor i = 1:size(CaF_Branches_3.Stat,2)
