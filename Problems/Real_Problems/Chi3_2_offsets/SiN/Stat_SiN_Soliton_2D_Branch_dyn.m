...
%    Delta_Start_Vector = linspace(1.25,4,NN)*SiN.Stat.In.kappa;
    ...
    Power_Start_Vector = [0.0325,0.0325];     
    Delta_Start_Vector = [3,6.05]*SiN.Stat.In.kappa;
...
Cust        = parcluster('local');
 Path          = strcat('/home/dp710/Data_Storage/SiN_Soliton_2_Vel_branches');
...
%     delete(gcp('nocreate'));
 %    Cust       = parcluster('local');
  %   parpool(20);
     ...
     mkdir(strcat(Path,'/Data_Start'));
     
     for  i_d =1:size(Res_Branch.Stat,2)
         
         Alph_Ind =    (letters(ii_matrix(i_p,i_d)+26*27));

         if~isfile(strcat(Path,'/Data_Start/',Alph_Ind{1},'.mat'))
             ...
             save(strcat(Path,'/Data_Start/',Alph_Ind{1},'.mat'),'Res');
         end
         i_p
     end
 ...
 for i_p = 1:size(SiN_Soliton,2)
     ...
     parfor  i_d =1:size(Res_Branch.Stat,2)
         ...
         if~isfile(strcat(Path,'/Data/',Alph_Ind{1},'.mat'))
             ...
             Res               = importdata(strcat(Path,'/Data_Start/',Alph_Ind{1},'.mat'));
             tic
