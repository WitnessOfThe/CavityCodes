function Sol = FullField_SingleProgection_dynamics_ForLongSave(Temp,Runge)   
...
%    Sol.Psie = complex(zeros(Temp.Par.dd,Temp.Space.N));
    ...
    Sol.PsiResolved     = complex(zeros(round(nt),5));
%    NRF        = round(1/dt/10);
...
%    nii = 0;
 %   phase_pump       = complex(0);
  ...
  %      pump_phase_substep(i)   = exp( 1i*Temp.Eq.Pump.delta*(t_zero +Runge.a(i)*dt));
    ...
    ind_Save = [find(Temp.In.m == Temp.In.Pump.m_p ),...
        find(Temp.In.m == 2*Temp.In.Pump.m_p - Temp.In.g),...
            find(Temp.In.m == 2*Temp.In.Pump.m_p + Temp.In.g),...
                    find(Temp.In.m == 2*Temp.In.Pump.m_p - 3*Temp.In.g),...
                        find(Temp.In.m == 2*Temp.In.Pump.m_p + 3*Temp.In.g)];

    Sol.mSaved = Temp.In.m(ind_Save);
...
%        Field =  Get_Field(N/2,F_e);
        Sol.PsiResolved(ni+1,:)     = F_e(Temp.Eq.mode_range(ind_Save)).*exp(1i*Temp.Eq.omega(ind_Save).*ni*dt);       
%        nii = nii +1;
