  ...
  struct5_T *Runge, struct6_T *Sol)
{
  ...
  /* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
  for (k = 0; k < 512; k++) {
    ind_s = Temp->Par.dt * (0.0 * Temp->Eq.L[k].re - (-Temp->Eq.L[k].im));
    im = Temp->Par.dt * (0.0 * Temp->Eq.L[k].im + -Temp->Eq.L[k].re);
    dcv[k].re = ind_s;
    dcv[k].im = im;
    if (im == 0.0) {
      dcv[k].re = muDoubleScalarExp(ind_s);
      dcv[k].im = 0.0;
    } else if (muDoubleScalarIsInf(im) && muDoubleScalarIsInf(ind_s) && (ind_s <
                0.0)) {
      dcv[k].re = 0.0;
  ...
  for (ni = 0; ni < i; ni++) {
    ...
    for (i3 = 0; i3 < 512; i3++) {
      ...
      q = b_d * Psi[i3].im;
      Psi[i3].re = ind_s * b_Runge - im * q;
      Psi[i3].im = ind_s * q + im * b_Runge;
    }

    Psi[0].re += exp_plus_omega_contents[0].re * d2;
    Psi[0].im += exp_plus_omega_contents[0].im * d2;
    for (i3 = 0; i3 < 512; i3++) {
    ...
    for (b_i2 = 0; b_i2 < i1; b_i2++) {
      ...
      for (i3 = 0; i3 < 512; i3++) {
        loop_ub = (b_i2 + 3 * i3) + 1;
        ind_s = 0.0 * exp_plus_omega_contents[loop_ub].re -
      ...
      Psi[0].re += exp_plus_omega_contents[b_i2 + 1].re * d2;
      Psi[0].im += exp_plus_omega_contents[b_i2 + 1].im * d2;
