...
%% Temp Constants
        ...
        L_L.Eq.h            = 1/2*L_L.In.kappa*L_L.In.H/coeff;%;%%;*sqrt(L_L.In.gamma/L_L.In.kappa)
         
        L_L.Eq.omega_j      =   L_L.In.omega_int/coeff- 1i*L_L.Eq.kappa/2;  
    
        L_L.Eq.gamma_Kerr   =  L_L.Eq.gamma_3*ones(1,L_L.Space.N);
        
        L_L.Eq.mask         =  ones(1,L_L.Space.N);%[ones(1,L_L.In.range),zeros(1,L_L.Space.N-L_L.In.range*2+1),ones(1,L_L.In.range-1)];
