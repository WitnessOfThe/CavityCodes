...
%     
    ...
    Res.Stat.Sol.Psi_o(2)  = coeff;%Res.CW.Stab.Vector(i_kp).V(1,i_Vp)*coeff;       %+ Res.CW.Stab.Vector(i_k(2)).V(1,i_c(2) )    
    %Res.Stat.Sol.Psi_e(2)  = Res.CW.Stab.Vector(i_Vp(1)).V(3,i_Vp)*coeff;     %+ Res.CW.Stab.Vector(i_k(2)).V(2,i_c(2) )
    
    Res.Stat.Sol.Psi_o(end)  = -coeff;%Res.CW.Stab.Vector(i_km).V(1,i_Vm)*coeff;       %+ Res.CW.Stab.Vector(i_k(2)).V(1,i_c(2) )    
  %  Res.Stat.Sol.Psi_e(end)  = Res.CW.Stab.Vector(i_Vm(1)).V(3,i_Vm)*coeff;     %+ Res.CW.Stab.Vector(i_k(2)).V(2,i_c(2) )
    ...
    V =0;% (imag(Res.CW.Stab.Value(i_kp(1),i_Vp)) - imag(Res.CW.Stab.Value(i_km(1),i_Vm)))/2/Res.Stat.In.mu_bl;
