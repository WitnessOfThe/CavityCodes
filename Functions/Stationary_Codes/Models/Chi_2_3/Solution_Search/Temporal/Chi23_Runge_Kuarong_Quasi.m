    function Sol = Chi23_Runge_Kuarong_Quasi(Temp,Runge)   
   % coder.gpu.kernelfun;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    g_1      =  G_theta.*exp(1i*Temp.Space.phi*Temp.Eq.g);
    g_2      =  G_theta.*exp(-1i*Temp.Space.phi*Temp.Eq.g);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 function k_e = Chi_3_LLE_Kuar_ins(F_Psi,Fac_plus,Fac_minus,t,Eq)     
    ...
    k_e(1:N)    = 1i*Fac_plus(1:N).*(Eq.gam2o.*fft(g_1.*conj(Psio).*Psie)+  Eq.gam3o.*fft((abs(Psio).^2 + 2*abs(Psie).^2).*Psio )   );%
    k_e(N+1:2*N)= 1i*Fac_plus(N+1:2*N).*(Eq.gam2e.*fft(g_2.*Psio.^2) +Eq.gam3e.*fft((2*abs(Psio).^2 + abs(Psie).^2).*Psie) );% 
