function Temporal = Full_Runge_Cubic_Reduced(F_e,dt,nt,NN,NNN,N,Runge,Delta,gamma_cubic,kappa,Delta_Laser,Pump,omega_j,Omega_start,File_name,h_delta,t_acc)
...
%    N_t      = 2^15;
    ...
    l_m                 = [50:10:100,104,110,112,120:10:310,312,320:10:330,336,340:10:350];%,300];%,350,400,450,500];%,550,600,650];
...
%    Temporal.E_t         = Amplitude;
    ...
    function k_e = Equation_with_Cubic(E_j,exp_plus_omega,g_c,exp_minus_omega)
         ...
         F_E_0_3             =      fft(6*abs(Q).^2.*real(Q))/N;%%+3*abs(Q).^2.*conj(Q)
         
         k_e              =          zeros(1,N);

         Nonlinear_term(mode_range)      = 1i*gamma_cubic(mode_range).*F_E_0_3(mode_range);%tt.*cl
        
             Pump_term(max_ind) =        g_c;        
         k_e(mode_range)    =  exp_plus_omega(mode_range).*( - kappa(mode_range)/2.* E_j(mode_range)+1i*Pump_term(mode_range) + Nonlinear_term(mode_range));
